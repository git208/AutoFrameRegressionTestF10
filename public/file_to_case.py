...
from public.parse_excel import ParseExcel
from public.parse_yaml import ParseYaml
from public.request_common import form_data_format_body,form_data_format_header
    ...
    def requests(self):
        if self.method == 'get':
            ...
            if self.type == 'form-data':
                ...
                LogCustom().logger().info(f'用例[{self.testCaseName}]请求内容类型有误，请检查用例')
                response = None
        else:
            LogCustom().logger().info(f'用例[{self.testCaseName}]请求方式有误，请检查用例')
            response = None
        return response
